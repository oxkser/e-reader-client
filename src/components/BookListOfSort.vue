...
<script>
import { searchbookbysort } from '@/api/e-reader'
...
export default {
  ...
  methods: {
    onLoad () {
      this.handleSearchBookBySort(this.sort)
    },
    ...
    // 处理模糊分页查询
    async handleSearchBookBySort (sort) {
      try {
        const res = await searchbookbysort(sort, this.current)
        setTimeout(() => {
          if (res.code === 1) {
